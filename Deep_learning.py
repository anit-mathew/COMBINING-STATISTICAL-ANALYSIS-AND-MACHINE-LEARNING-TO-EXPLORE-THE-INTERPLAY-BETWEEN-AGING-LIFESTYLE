...
def plot_variable_importance(model, feature_names):
    if hasattr(model, 'fc1') and isinstance(model.fc1, nn.Linear):
        ...
        plt.title('Variable Importance Plot of DNN for original data')
        plt.show()

        return feature_importance  # Return the numerical values
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):
    ...
    with torch.no_grad():
        ...
        print(f"F1 Score on test set (fold {fold + 1}): {f1}")

        # Plot variable importance
        if fold == num_folds - 1:  # Plot only for the last fold
            variable_importance = plot_variable_importance(model, feature_names=['gender', 'hypertension', 'heart_disease', 'ever_married', 'work_type', 'Residence_type', 'avg_glucose_level', 'bmi', 'smoking_status'])
